...
import './HomePage.css';
import { Home, User, MessageSquare, CalendarDays, Tickets, Settings, Heart, LogOut } from 'lucide-react';
  ...
  return (
    <nav className="navbar">
      <ul>
        ...
        <li><Link to="/messages" className="link"><div>Messages</div><MessageSquare size={24} /></Link></li>
        <li>
          <div onClick={toggleDarkMode} className="settings link">
            <p>Settings </p>
            <Settings size={24} />
...
const PostsFeed = ({ posts, handleLikeToggle, toggleModal, name }) => {
      ...
      >
        {displayedPosts.map((post) => (
          <div key={post.id} className="post">
            ...
            >
              <Heart size={16} className="heart"/> {post.likes}
            </button>
          </div>
        ))}
        {loading && (
